...
sub tokenize {
	...
	foreach my $n (@res) {
  		if ($n eq "(") { $brackets++; }
  		elsif ($n eq")"){ $brackets--; if ($brackets<0) { return "none"; }	}
	}
    ...
    return "none" unless ($res[0]=~ /^(\d+\.\d*|\d+)$/ or $res[0] eq "(" or $res[0] eq "+" or $res[0] eq "-" );
	$res[0]="U+" if ($res[0] eq "+");
	$res[0]="U-" if ($res[0] eq "-");
	...
	for (my $i=0; $i<=$#res; $i++){
		given ($res[$i]) {
			when (/^(\d+\.\d*|\d+)$/) { # элемент содержит число
				if ($pre==2 and $res[$i-1] eq ")") { return "none"; }
				 return "none" if ($pre==5);
				$pre=5;
			}
			when ([ '(', ')']) {
				if ($pre==5 and $res[$i] eq "(") { return "none"; }
				$pre=2;
			} 
			when ([ '+','-']){
				my $it=3; 
	...
	return "none" unless ($res[$#res]=~ /^(\d+\.\d*|\d+)$/ or $res[$#res] eq ")");
	return \@res;
}
